...
public class Character1 extends JPanel implements ActionListener, KeyListener, Runnable {
    ...
    public Character1(){
        ...
        this.bulletHitbox = new Hitbox(900, 50, 30, 600);
...
//        g.drawImage(imagenes.get(Type.BG),0,0,null);
        ...
        for (int i = 0; i < bulletX.length; i++){
            if(isShot[i] && imgActual== imagenes.get(Type.DERECHA)){
                ...
                g.fillRect(hitboxes.get(i).getX(), hitboxes.get(i).getY(), 30, 5);
        ...
        g.fillRect(bulletHitbox.getX(), bulletHitbox.getY(), 30, 600);
    ...
    public void collision(){
        if(hitboxes != null){
            for(Hitbox hit : hitboxes){
                if(hit.bounds().intersects(bulletHitbox.bounds())){
                    auxHit = hit;
                    System.out.println("Colision!");
                }
            }
            hitboxes.remove(auxHit);
        }
    }
